...
function createMemoryAccountStore() {
  ...
  return {
    kind: 'memory',
    async createAccount(account) { if (items.has(account.id)) throw new AccountError('ACCOUNT_EXISTS', '该邮箱已注册。', 409); items.set(account.id, { ...account }); return { ...account }; },
    async readAccount(id) { const item = items.get(id); return item?.type === 'account' ? { ...item } : null; },
    async saveAccount(account) { items.set(account.id, { ...account }); return { ...account }; },
...
function createCosmosAccountStore(options = {}) {
  ...
  return {
    ...
    async saveAccount(account) { const clean = { ...account }; delete clean._rid; delete clean._self; delete clean._attachments; delete clean._ts; return (await (await container()).items.upsert(clean)).resource; },
