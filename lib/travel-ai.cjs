...
const SKILL_CONTRACT = require('./travel-ai-contract.cjs');
...
function responseSchemaForContext(context) {
  if (!['apply_answer_to_current_plan', 'edit_selected_text', 'edit_module'].includes(context?.action)) return SKILL_CONTRACT.RESPONSE_SCHEMA;
...
function validateRequest(body) {
  ...
  const editSelectedText = body.action === 'edit_selected_text';
  const editModule = body.action === 'edit_module';
  ...
  const editableFields = new Set(['title', 'kicker', 'intro', 'overview', 'highlights', 'routes', 'food', 'accommodation', 'daily_stays', 'packing', 'quote']);
  const moduleFields = editModule && Array.isArray(body.module_context?.fields)
    ? [...new Set(body.module_context.fields.map(field => String(field || '').trim()).filter(field => editableFields.has(field)))].slice(0, 3)
    : [];
  const moduleContext = editModule && body.module_context && typeof body.module_context === 'object'
    ? {
        fields: moduleFields,
        field_label: String(body.module_context.field_label || '').trim().slice(0, 80),
        text: String(body.module_context.text || '').trim().slice(0, 1800),
        suggestion: String(body.module_context.suggestion || '').trim().slice(0, 200)
      }
    : null;
  if (editModule && (!moduleContext?.fields.length || !moduleContext.text || !moduleContext.suggestion || !body.current_plan || typeof body.current_plan !== 'object')) {
    throw new TravelAIError('INVALID_MODULE_CONTEXT', 'AI 建议所属栏目内容无效，请刷新页面后重试。', 400);
  }
  const selectionContext = editSelectedText && body.selection_context && typeof body.selection_context === 'object'
    ? {
        text: String(body.selection_context.text || '').trim().slice(0, 800),
        field: String(body.selection_context.field || '').trim(),
        field_label: String(body.selection_context.field_label || '').trim().slice(0, 80),
        item_index: Number.isInteger(body.selection_context.item_index) && body.selection_context.item_index >= 0 && body.selection_context.item_index < 30
          ? body.selection_context.item_index : null
      }
    : null;
  if (editSelectedText && (!selectionContext?.text || !editableFields.has(selectionContext.field))) {
    throw new TravelAIError('INVALID_SELECTION_CONTEXT', '引用的行程文字无效，请重新选择。', 400);
  }
  if (editSelectedText && (!body.current_plan || typeof body.current_plan !== 'object')) {
    throw new TravelAIError('INVALID_SELECTION_CONTEXT', '找不到引用文字所属的当前行程。', 400);
  }
  if (editSelectedText && Array.isArray(body.current_plan[selectionContext.field]) && selectionContext.item_index == null) {
    throw new TravelAIError('INVALID_SELECTION_CONTEXT', '找不到引用文字在当前行程中的位置，请重新选择。', 400);
  }
  return {
    ...
    action: applyAnswer ? 'apply_answer_to_current_plan' : editModule ? 'edit_module' : editSelectedText ? 'edit_selected_text' : null,
...
async function callMockProvider(context) {
  ...
  if (context.action === 'edit_module') {
    const current = context.current_plan || {};
    const suggestion = context.module_context.suggestion;
    const patchFields = ['destination', 'duration_days', 'waypoint', 'title', 'kicker', 'intro', 'overview', 'highlights', 'routes', 'food', 'accommodation', 'daily_stays', 'packing', 'quote'];
    const plan = Object.fromEntries(patchFields.map(key => [key, null]));
    for (const field of context.module_context.fields) {
      const source = current[field];
      if (Array.isArray(source)) {
        const updated = source.length ? [...source] : ['待补充'];
        updated[0] = `${updated[0]}（${suggestion}）`;
        plan[field] = updated;
      } else {
        plan[field] = `${String(source || '')}${source ? '；' : ''}${suggestion}`;
      }
    }
    return {
      value: {
        mode: 'PLAN', intent: 'modify_current', assistant_message: `已根据 AI 建议重新整理“${context.module_context.field_label}”。`,
        ...
        proposal: {
          ...
          preserve_unmentioned: true, summary: `按“${suggestion}”修改${context.module_context.field_label}`,
  ...
  if (context.action === 'edit_selected_text') {
    const current = context.current_plan || {};
    const { field, text, item_index: itemIndex } = context.selection_context;
    const replacement = `${text}（已按要求修改）`;
    const source = current[field];
    const updated = Array.isArray(source)
      ? source.map((item, index) => index === itemIndex ? String(item).replace(text, replacement) : item)
      : String(source || '').replace(text, replacement);
    const plan = Object.fromEntries(['destination', 'duration_days', 'waypoint', 'title', 'kicker', 'intro', 'overview', 'highlights', 'routes', 'food', 'accommodation', 'daily_stays', 'packing', 'quote'].map(key => [key, null]));
    if (Object.hasOwn(plan, field)) plan[field] = updated;
    return {
      value: {
        mode: 'PLAN', intent: 'modify_current', assistant_message: `已按你的要求定向修改“${text.slice(0, 24)}”。`,
        requires_confirmation: true, missing_fields: [],
        proposal: {
          operation: 'modify_current', target_id: current.id || null, destination, duration_days: null,
          preserve_unmentioned: true, summary: `仅修改${context.selection_context.field_label || field}中的引用文字`,
          patch: { profile: null, plan }
        },
        results: [], citations: [], warnings: ['当前使用 LLM_PROVIDER=mock，仅用于自动化测试。']
      },
...
async function createTravelAI(options = {}) {
  ...
  const validator = await import('./validate-travel-response.mjs');
  return {
    ...
    async chat(body, env = process.env) {
      ...
      if (accommodationRequest && !['apply_answer_to_current_plan', 'edit_selected_text', 'edit_module'].includes(context.action) && !hasAccommodationBudget(context)) {
        return {
      ...
      if (context.action === 'edit_selected_text') {
        const field = context.selection_context.field;
        const plan = value.proposal?.patch?.plan;
        if (value.intent !== 'modify_current' || value.proposal?.operation !== 'modify_current' || !plan || plan[field] == null
          || JSON.stringify(plan[field]) === JSON.stringify(context.current_plan?.[field])) {
          throw new TravelAIError('INVALID_PLAN_UPDATE', 'AI 未能生成有效的定向文字修改，请换一种说法重试。');
        }
        for (const key of ['destination', 'duration_days', 'waypoint', 'title', 'kicker', 'intro', 'overview', 'highlights', 'routes', 'food', 'accommodation', 'daily_stays', 'packing', 'quote']) {
          if (key !== field) plan[key] = null;
        }
      }
      if (context.action === 'edit_module') {
        const plan = value.proposal?.patch?.plan;
        const fields = context.module_context.fields;
        const changed = plan && fields.some(field => plan[field] != null
          && JSON.stringify(plan[field]) !== JSON.stringify(context.current_plan?.[field]));
        if (value.intent !== 'modify_current' || value.proposal?.operation !== 'modify_current' || !changed) {
          throw new TravelAIError('INVALID_PLAN_UPDATE', 'AI 未能生成有效的栏目修改，请换一个建议重试。');
        }
        for (const key of ['destination', 'duration_days', 'waypoint', 'title', 'kicker', 'intro', 'overview', 'highlights', 'routes', 'food', 'accommodation', 'daily_stays', 'packing', 'quote']) {
          if (!fields.includes(key)) plan[key] = null;
        }
      }
      if (context.action === 'apply_answer_to_current_plan') {
        const plan = value.proposal?.patch?.plan;
