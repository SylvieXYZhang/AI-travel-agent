...
function responseSchemaForContext(context) {
  if (!['apply_answer_to_current_plan', 'edit_selected_text'].includes(context?.action)) return SKILL_CONTRACT.RESPONSE_SCHEMA;
...
function validateRequest(body) {
  ...
  return {
    ...
    action: applyAnswer ? 'apply_answer_to_current_plan' : editSelectedText ? 'edit_selected_text' : null,
...
async function createTravelAI(options = {}) {
  ...
  return {
    ...
    async chat(body, env = process.env) {
      ...
      if (accommodationRequest && !['apply_answer_to_current_plan', 'edit_selected_text'].includes(context.action) && !hasAccommodationBudget(context)) {
        return {
