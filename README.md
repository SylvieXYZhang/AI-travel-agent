...
> [!IMPORTANT]
> 当前版本已接入服务端大模型问答链路：配置 API Key 后，通过 OpenAI Responses API、Web Search 与可选的小红书只读检索生成结构化回答和旅行计划提案。专用酒店、地图、预算服务与云端账号仍未接入。
...
- 使用键盘 `←` `→` 切换目的地。
- 在手机上横向滑动书页切换记录。
- 点击手账右上角“编辑行程”后，可直接在渲染后的手账页面修改文字，设置十种预设文字颜色，并通过折叠式“插入”菜单添加链接、网络或本地图片和可编辑表格；保存后刷新页面仍会保留。
- 行程模板包含住宿推荐与每日住宿安排；每日住宿会标记住宿地点及每晚价格区间，AI 新建或修改行程时也会同步生成住宿字段。
- 页面启动时会按城市名称去除重复目的地；住宿及价格字段默认使用人民币，除非用户明确指定其他币种。
- 使用右下角 `+` 展开 AI 对话。
- 通过真实大模型问答、检索目的地信息、推荐目的地或生成计划变更提案。
- AI 回答后可点击“根据回答修改行程”；系统会读取当前城市原行程与新建议，再生成完整修改方案并刷新旅行记录。点击该按钮即表示同意执行本次修改。
- 在执行写入前确认或取消 AI 提案。
- 上传个人头像；未上传时显示 `ME`。
...
项目支持对接开源的 [xpzouying/xiaohongshu-mcp](https://github.com/xpzouying/xiaohongshu-mcp)（Apache-2.0）。本项目只调用其搜索与笔记详情 REST API，不调用发布、评论、点赞或收藏接口。

1. 按上游说明启动服务并完成一次扫码登录，确认 `http://127.0.0.1:18060/health` 可访问。
2. 在 `.env` 中启用：
...
当问题明确包含“小红书”“XHS”“RedNote”等关键词时，后端会先检索相关图文笔记和详情，把它们作为带来源的证据交给模型。未启用、登录过期或服务超时时，会自动改用 `site:xiaohongshu.com <关键词>` 搜索公开索引，并明确提示覆盖范围可能不完整。请仅在遵守小红书条款、适用法律及上游项目许可的前提下使用。
...
| 小红书专用数据源 | 可选接入 `xiaohongshu-mcp`，仅只读搜索与详情；需本地登录并自行评估平台合规性 |
| 地图地点、距离与路线优化 | 规划中 |
| 预算和实时价格 | 后续版本 |
| 账号与云端保存 | 规划中 |
