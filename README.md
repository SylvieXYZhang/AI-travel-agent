...
- **AI 生成后仍由你做主**：手账可直接编辑文字、颜色、链接、图片和表格，并保存在当前登录账号中。
...
| 账号保存 | AI 创建、AI 修改和手动编辑的行程均保存到当前账号，重新登录或换设备后可继续 |
...
## 使用 Azure Cosmos DB 保存账号

项目在设置 `COSMOS_ENDPOINT` 后会自动将账号、登录会话、加密后的模型配置、旅行偏好和账号页面状态写入 Azure Cosmos DB。请先自行创建兼容 SQL API 的数据库、`accounts` 与 `sharedPlans` 容器，并为运行应用的 Microsoft Entra ID 身份授予数据读写权限。相关配置包括：

```text
COSMOS_ENDPOINT=https://<account>.documents.azure.com:443/
COSMOS_DATABASE=voyageai
COSMOS_ACCOUNT_CONTAINER=accounts
COSMOS_CONTAINER=sharedPlans
ACCOUNT_ENCRYPTION_KEY=<32-byte-base64-key>
```

已有 App Service 和 Cosmos DB 资源时，可登录 Azure CLI 后使用仓库内的应用部署脚本：

```powershell
az login
.\scripts\deploy-app-only.ps1 -ResourceGroup 'rg-voyageai-prod' -AppName '<app-name>'
```

该脚本只打包并更新应用，不创建或修改 Azure 基础设施。部署后会检查 `/api/health`，确认账号存储为 Cosmos DB 且页面状态持久化接口已启用。随后可完成一次注册、重启 App Service，再用同一邮箱和密码登录，以验证账号持久化。
...
- 账号状态中的分享编辑密钥也会使用 `ACCOUNT_ENCRYPTION_KEY` 加密后保存。
- 登录后的旅行手账、旅行偏好和头像保存在当前账号的服务端状态中；不同账号相互隔离，未保存状态的新账号从统一冷启动页面开始。
- 从旧版本升级时，浏览器中的本地手账、偏好和头像会在首次登录后迁移到当前账号，并仅在服务端保存成功后清除旧副本。
- 用户主动分享的单页规划会保存到配置的数据存储；只读链接公开可读，可编辑链接持有者可修改。
- 用户上传到手账或头像的本地图片会随账号状态保存；单张图片和账号总状态均有大小限制。
